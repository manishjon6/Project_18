...
class SentimentAnalysis:
	...
	def DownloadData(self, keyword, tweets):
		...
		# authenticating
		consumerKey = 'MM2gPHXgHgpQ3VTuxy4JvnP'
		consumerSecret = 'EZEgJrpVeLLdmDIjXmvS0WAfCDetLZN8yfIxOJURiNe0NHsZDm'
		accessToken = '1538924473755537408-qHOakmkKhlENJVERSpse8Asm1rQTCt'
		accessTokenSecret = 'OLY3TZsCXittGUaveX3aEMogd2c6A5Le0bxBbpTq0SKJ8'
	...
	def plotPieChart(self, positive, wpositive, spositive, negative, wnegative, snegative, neutral, keyword, tweets):
		...
		colors = ['yellowgreen', 'lightgreen', 'darkgreen',
				'gold', 'red', 'lightsalmon', 'purple']
		wp = {'linewidth':2, 'edgecolor':'black'}

		explode = [0,0,0,0,0,0,0]
		patches, texts = plt.pie(sizes, colors=colors, startangle=90)
		texts = plt.pie(sizes, labels=labels, colors= colors, explode= explode, autopct='%1.2f%%', shadow=True, textprops={'fontsize': 16})
		plt.legend(fontsize=20)
		plt.legend(title = 'catagories')
		plt.axis('equal')
		plt.title("Result Analysis Of Twitter Sentiments", fontsize = 20)
		plt.tight_layout()
		strFile = r"C:\Users\manis\Desktop\Project_18\SentimentAnalysis-master\static\images\plot1.png"
	...
	# 	# 		'%]', 'Weakly Negative [' + str(wnegative) + '%]',
	# 	# 		'Strongly Negative [' + str(snegative) + '%]']
	# 	plt.rcParams["figure.figsize"] = [10.5, 6.00]
	# 	plt.rcParams["figure.autolayout"] = False
	# 	df = pd.DataFrame(
	# 		dict(
	# 			labels=["S neg", "Neutral", "Neg", "W Neg", "Pos", "W Pos", "S Pos"],
	# 			sizes = [snegative, neutral, negative, wnegative, positive, wpositive, spositive]
	# 		)
	# 	)
		...
		# sort dataframe
		# df.sort_values('sizes')
		#
		# # make barplot and sort bars
		# sns.barplot(x='labels',
		# 			y="sizes", data=df,
		# 			order=df.sort_values('sizes').sizes)


		# plt.subplot(121)
		# plt.bar('labels','sizes', data=df, color='green')
		# plt.subplot(122)
		# df_sorted = df.sort_values('sizes')
		# plt.bar('labels','sizes',data= df_sorted, width = 0.4 , color='blue')
		# sizes = [positive, wpositive, spositive,
		# 		neutral, negative, wnegative, snegative]
		#
		# colors = ['yellowgreen', 'lightgreen', 'darkgreen',
		# 		'gold', 'red', 'lightsalmon', 'darkred']
		# plt.bar(labels,range(len(labels)),alpha= 0.5)
		# #patches, texts = plt.pie(sizes, colors=colors, startangle=90)
		# texts = plt.pie(sizes, labels=labels, colors= colors)
